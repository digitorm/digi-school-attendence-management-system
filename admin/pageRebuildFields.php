<?php
	...
	/*
		$schema: [ tablename => [ fieldname => [ appdigi => '...', 'db' => '...'], ... ], ... ]
	*/

	/* application schema as created in AppDigi */
	$schema = array(   
		'students' => array(   
			'regno' => array('appdigi' => 'VARCHAR(40) not null primary key '),
			'name' => array('appdigi' => 'VARCHAR(100) not null '),
			'course' => array('appdigi' => 'INT unsigned ')
		),
		'units' => array(   
			'id' => array('appdigi' => 'INT unsigned not null primary key auto_increment '),
			'name' => array('appdigi' => 'VARCHAR(40) ')
		),
		'courses' => array(   
			'id' => array('appdigi' => 'INT unsigned not null primary key auto_increment '),
			'name' => array('appdigi' => 'VARCHAR(40) ')
		...
		'attendance' => array(   
			'student' => array('appdigi' => 'VARCHAR(40) '),
			'regno' => array('appdigi' => 'VARCHAR(40) '),
			'week' => array('appdigi' => 'VARCHAR(40) '),
			'date' => array('appdigi' => 'DATE '),
			'unit' => array('appdigi' => 'INT unsigned '),
			'attended' => array('appdigi' => 'VARCHAR(40) '),
			'id' => array('appdigi' => 'INT unsigned not null primary key auto_increment ')
		)
	);
	...
	function fix_field($fix_table, $fix_field, $schema, &$qry){
		...
		if($row = db_fetch_assoc($res)){
			// modify field
			$qry = "alter table `{$fix_table}` modify `{$fix_field}` {$def['appdigi']}";
			sql($qry, $eo);

			// remove unique from db if necessary
			if($row['Key'] == 'UNI' && !stripos($def['appdigi'], ' unique')){
		...
		// create field
		$qry = "alter table `{$fix_table}` add column `{$fix_field}` {$schema[$fix_table][$fix_field]['appdigi']}";
		sql($qry, $eo);
		return 2;
	}
	...
	foreach($table_captions as $tn => $tc){
		...
		if($res){
			while($row = db_fetch_assoc($res)){
				if(!isset($schema[$tn][$row['Field']]['appdigi'])) continue;
	...
	if($fix_all){
		foreach($schema as $tn => $fields){
			foreach($fields as $fn => $fd){
				if(prepare_def($fd['appdigi']) == prepare_def($fd['db'])) continue;
...
<table class="table table-responsive table-hover table-striped">
	<thead><tr>
		...
		<th><?php echo $Translation['AppDigi definition'] ; ?></th>
	...
	<?php foreach($schema as $tn => $fields){ ?>
		...
		<?php foreach($fields as $fn => $fd){ ?>
			<?php $diff = ((prepare_def($fd['appdigi']) == prepare_def($fd['db'])) ? false : true); ?>
			...
			<tr class="<?php echo ($diff ? 'warning' : 'field_ok'); ?>">
				...
				<td><?php echo $fn; ?></td>
				<td class="<?php echo ($diff ? 'bold text-success' : ''); ?>"><?php echo $fd['appdigi']; ?></td>
