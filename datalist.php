...
class DataList{
	...
	// handle user commands ...
		...
		elseif($delete_x != ''){
			...
			if($delete_res){
				...
				if($Embedded){
					$this->HTML .= '<script>$j(function(){ setTimeout(function(){ AppDigi.closeParentModal(); }, 2000); })</script>';
				}
			}
		}
	...
	function tv_tools(){
		...
		<div class="pull-right flip btn-group vspacer-md" id="tv-tools">
			<button title="<?php echo html_attr($Translation['previous column']); ?>" type="button" class="btn btn-default tv-scroll" onclick="AppDigi.TVScroll().less()"><i class="glyphicon glyphicon-step-backward"></i></button>
			<button title="<?php echo html_attr($Translation['next column']); ?>" type="button" class="btn btn-default tv-scroll" onclick="AppDigi.TVScroll().more()"><i class="glyphicon glyphicon-step-forward"></i></button>
		...
		<script>
			$j(function(){
				...
				var show_column = function(col_class, show){
					...
					AppDigi.TVScroll().reset();

					col_cookie(col_class, show);
				}

				/* initiate TVScroll */
				AppDigi.TVScroll().less();
