...
function attendance_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	<script>
		...
		AppDigi.current_student__RAND__ = { text: "", value: "<?php echo addslashes($selected_id ? $urow['student'] : $filterer_student); ?>"};
		AppDigi.current_unit__RAND__ = { text: "", value: "<?php echo addslashes($selected_id ? $urow['unit'] : $filterer_unit); ?>"};
		...
		jQuery(function() {
			...
			}, 10); /* we need to slightly delay client-side execution of the above code to allow AppDigi.ajaxCache to work */
		});
			...
			$j("#student-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppDigi.current_student__RAND__.value, t: 'attendance', f: 'student' },
				...
				AppDigi.current_student__RAND__.value = e.added.id;
				AppDigi.current_student__RAND__.text = e.added.text;
			...
			$j("#unit-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppDigi.current_unit__RAND__.value, t: 'attendance', f: 'unit' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				AppDigi.current_unit__RAND__.value = e.added.id;
				AppDigi.current_unit__RAND__.text = e.added.text;
	...
	if($_REQUEST['Embedded']){
		$backAction = 'AppDigi.closeParentModal(); return false;';
	}else{
	...
	if($dvprint){
		...
		$templateCode .= "\t\t\turl: 'attendance_autofill.php?rnd1=$rnd1&mfk=student&id=' + encodeURIComponent(AppDigi.current_student{$rnd1}.value),\n";
